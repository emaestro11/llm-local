...
- `docs/phase-2-new-listing-plan.md` — **/office-hours session 2026-04-21, eng-cleared 2026-05-08:** pivoted from BTC 15m prompt iteration to new-listing scalping (deterministic shorts + LLM qualifier). Contains edge hypothesis, chosen approach, all 5 must-fix items resolved with locked architecture decisions (1A–7A). **Read before coding Phase 2.**
- Full design doc: `~/.gstack/projects/emaestro11-llm-local/esteban-main-design-20260421-221849.md` (all 19 reviewer concerns + resolutions).
- Test plan: `~/.gstack/projects/emaestro11-llm-local/esteban-main-eng-review-test-plan-20260508-162207.md` (36 paths, 4 regression-critical, 5 integration tests).
...
- **Strategic pivot (2026-04-21, via /office-hours):** path D chosen. New-listing scalping. Edge: Binance listing FOMO pump + retail-trap reversal in first 60 min, arbitrage-resistant because sellers ARE the smart money (VC exit liquidity). 70% dump rate, 2025–2026 sources.
- **Eng review (2026-05-08):** all 5 must-fix blockers resolved with locked architecture (1A: new ListingTrade table; 2A: new qualify_setup() function; 3A: entry-relative exit math; 4A: 4 Phase 0 spikes BLOCKING; 5A: raw OHLCV + derived features, no TA on listing 1m; 6A: adverse stop calibrated from The Assignment; 7A: live execution scoped after timing verification).
- Kill criterion: 30 qualified short trades, WR > 55%, Pearson r > 0.1.
- **Status: ENG CLEARED, ready to execute Phase 0.**
- Next session pick-up: **execute Phase 0 in this order** (`docs/phase-2-new-listing-plan.md` § Immediate Next Steps): The Assignment (5 listings, 30 min) → ccxt 1m depth spike (10 min) → listings registry source spike (15 min) → perp-listing timing spike (10 min). Pattern visible on <3/5 → KILL the direction. All 4 pass → Lane A (data) → Lanes B+C in parallel (strategy + qualifier) → Lane D (orchestration).
